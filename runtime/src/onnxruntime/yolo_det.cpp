...
std::vector<caiwei::image::Box> caiwei::context::DetONNXRuntimeContext::run(const caiwei::media::ImageFrame& image) {
    ...
    caiwei::image::transpose(output_data, out_dst.data(), signalResultNum, strideNum);
    ...
    for (int index = 0; index < strideNum; ++index) {
        ...
        caiwei::image::max_loc(scores_pos, this->class_size, max_score, max_class);
    ...
    return caiwei::image::nms_boxes(ret, this->iou_threshold);
}
